...
from Lesson1.Keyboards.prof_keyboards import make_row_keyboard
...
async def grade_chosen(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f'Вы выбрали {message.text.lower()} уровень. Ваша профессии {user_data.get("chosen_prof")}'
    )
    await state.set_state(ChoiceProfNames.choice_prof_payments)
    await message.answer(
        text='Сколько хочешь денег?',
        reply_markup=make_row_keyboard(available_prof_payments)
    )

@router.message(ChoiceProfNames.choice_prof_payments, F.text)
async def payment_chosen(message: types.Message, state: FSMContext):
    user_data = await state.get_data()
    chosen_payment = message.text.lower()  # Получаем выбранную зарплату из сообщения
    await message.answer(
        f'Вы выбрали зарплату {chosen_payment} для профессии {user_data.get("chosen_prof")} и уровня {user_data.get("chosen_grade")}. При наличии подходящих вакансий мы с вами свяжемся',
